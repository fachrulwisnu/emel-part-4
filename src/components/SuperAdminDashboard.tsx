...
// Full List of System AI Models available for allocation
...
export const SuperAdminDashboard: React.FC = () => {
  ...
  const [mainNav, setMainNav] = useState<'analytics' | 'management'>('management');

  // Tenant Data & Loading State
  ...
  const handleSaveTenant = async (e: React.FormEvent) => {
    ...
    // Ensure ai_models is non-empty array
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/20 text-blue-300 border border-blue-400/30 rounded-full text-xs font-semibold mb-2">
              ...
              <span>Super Admin Multi-Tenant SaaS Control Center</span>
            </div>
            <h1 className="text-2xl font-bold tracking-tight">Enterprise Multi-Tenant Management</h1>
            <p className="text-sm text-slate-300 mt-1">
              Atur Multi-Select AI Routing, alokasi token, fitur per divisi, dan transaksi user admin divisi.
            </p>
          </div>

          <button
            onClick={() => { loadTenants(); loadGlobalAnalytics(); }}
            disabled={isLoading}
      ...
      {/* TOP SIDEBAR / NAVIGATION SWITCHER MENU */}
      <div className="bg-white rounded-2xl border border-slate-200 p-2 flex items-center gap-2 shadow-xs">
        <button
          onClick={() => setMainNav('management')}
          className={`flex-1 py-3 px-4 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer ${
            mainNav === 'management'
        ...
        >
          ...
          <span>1. Tenant & User Management</span>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${
            mainNav === 'management' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-700'
          }`}>
            {tenants.length} Divisi
          </span>
        </button>

        <button
          onClick={() => setMainNav('analytics')}
          className={`flex-1 py-3 px-4 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer ${
            mainNav === 'analytics'
              ? 'bg-blue-600 text-white shadow-md'
              : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
        ...
        >
          ...
          <span>2. Dashboard Analytics & AI Intelligence</span>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${
            mainNav === 'analytics' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-700'
          }`}>
            {stats.totalEmails} Email
          </span>
        </button>
      </div>

      {/* ========================================================================= */}
      {/* MENU 1: TENANT & USER MANAGEMENT (CRUD TABEL DIVISI & ADMIN USER)        */}
      {/* ========================================================================= */}
      {mainNav === 'management' && (
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Kelola alokasi model AI, fitur workflow, serta kredensial login User Admin terisolasi per divisi.
              </p>
            </div>
      ...
      {/* ========================================================================= */}
      {/* MENU 2: DASHBOARD ANALYTICS (CHART & GLOBAL SYSTEM STATS)                 */}
      {/* ========================================================================= */}
      {mainNav === 'analytics' && (
        <div className="space-y-6">
          ...
          {/* Email Volume Breakdown Charts per Division */}
      ...
      {/* ========================================================================= */}
      {/* REDESIGNED MODAL 'TAMBAH / EDIT DIVISI' (INCLUDE CRUD USER ADMIN)        */}
      {/* ========================================================================= */}
      {editingTenant && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-sm p-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Apakah Anda yakin ingin menghapus divisi ID #{deleteConfirmId}? All associated data and user accounts will be removed.
              </p>
            </div>
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs shadow-xs"
              >
                Hapus
              </button>
            </div>
